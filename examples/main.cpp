...
enum class bar_enum { FOO, BAR, QUUX };
...
int main() {
  ...
  std::cout << ctti::pretty_function::value<decltype(&foo::i), &foo::i>() << " -> "
            << ctti::nameof<CTTI_STATIC_VALUE(&foo::i)>() << std::endl;
  std::cout << ctti::pretty_function::value<decltype(bar_enum::FOO), bar_enum::FOO>() << " -> "
            << ctti::nameof<CTTI_STATIC_VALUE(bar_enum::FOO)>() << std::endl;
