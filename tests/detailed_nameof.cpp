...
constexpr std::string_view ctti_nameof(CTTI_STATIC_VALUE(Enum::Value)) {
  return "Enum::Value";
}

constexpr std::string_view ctti_nameof(CTTI_STATIC_VALUE(Namespace::Struct::Enum::Value)) {
...
TEST_CASE("detailed_nameof") {
  SECTION("full_name") {
    ...
    REQUIRE(detailed_nameof<CTTI_STATIC_VALUE(&Namespace::Struct::member)>().full_name() ==
    ...
    REQUIRE(detailed_nameof<CTTI_STATIC_VALUE(Namespace::Struct::Enum::Value)>().full_name() ==
            "Namespace::Struct::Enum::Value");
#else
    // More flexible test for non-enum-aware compilers
    const auto full_name = detailed_nameof<CTTI_STATIC_VALUE(Namespace::Struct::Enum::Value)>().full_name();
    REQUIRE((full_name == "(Namespace::Struct::Enum)0" || full_name == "Namespace::Struct::Enum::Value" ||
             full_name.find("Namespace") != std::string::npos));
  ...
  SECTION("full_homogeneous_name") {
    ...
    REQUIRE(detailed_nameof<CTTI_STATIC_VALUE(&Namespace::Struct::member)>().full_homogeneous_name() ==
            "Namespace::Struct::member");
    REQUIRE(detailed_nameof<CTTI_STATIC_VALUE(Enum::Value)>().full_homogeneous_name() == "Enum::Value");

#ifdef CTTI_HAS_ENUM_AWARE_PRETTY_FUNCTION
    REQUIRE(detailed_nameof<CTTI_STATIC_VALUE(Namespace::Struct::Enum::Value)>().full_homogeneous_name() ==
            "Namespace::Struct::Enum::Value");
#else
    // More flexible test for non-enum-aware compilers
    const auto full_homogeneous_name =
        detailed_nameof<CTTI_STATIC_VALUE(Namespace::Struct::Enum::Value)>().full_homogeneous_name();
    REQUIRE((full_homogeneous_name == "(Namespace::Struct::Enum)0" ||
             full_homogeneous_name == "Namespace::Struct::Enum::Value" ||
             full_homogeneous_name.find("Namespace") != std::string::npos));
#endif  // CTTI_HAS_ENUM_AWARE_PRETTY_FUNCTION
  }
...
#else
    // More flexible test for non-enum-aware compilers
    const auto name = detailed_nameof<CTTI_STATIC_VALUE(Namespace::Struct::Enum::Value)>().name();
    REQUIRE((name == "Enum)0" || name == "Value" || name == "0" || name.find("Value") != std::string::npos ||
...
#else
    // More flexible test for non-enum-aware compilers
    const auto qualifier = detailed_nameof<CTTI_STATIC_VALUE(Namespace::Struct::Enum::Value)>().qualifier(0);
    REQUIRE((qualifier == "(Namespace" || qualifier == "Namespace" ||
             qualifier.find("Namespace") != std::string::npos || qualifier.empty() || qualifier[0] == '(' ||
             qualifier.find("Enum") != std::string::npos));
#endif  // CTTI_HAS_ENUM_AWARE_PRETTY_FUNCTION
  }
